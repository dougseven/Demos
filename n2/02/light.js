/* Indicator light
 * In this demo I create an indicator light that subscribes to the 
 * message stream of an ambient light sensor (sensor.js). When 
...
var cmdTag = 'n2demo_light';

board = new five.Board({ port: "/dev/cu.usbmodem1421" });
...
service.connect(indicatorLight, function(err, session, indicatorLight) {
    if (err) { return console.log('Failed to connect D7 ndicator Light: ' + err); }
...
LightManager.prototype.isRelevant = function(message) {
    return (message.is('_color') || message.is('_ambientLight'));
};
...
LightManager.prototype.isCommand = function(message) {
    return message.is('_ambientLight')
};
